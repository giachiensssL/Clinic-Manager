...
with open('app/api/v1/receptionist.py', 'w', encoding='utf-8') as f:
    f.write(api_content)
...
# Replace receptionist_dashboard with receptionist
if 'from app.api.v1 import receptionist_dashboard' in content:
    content = content.replace('from app.api.v1 import receptionist_dashboard', 'from app.api.v1 import receptionist')
    content = content.replace('router.include_router(receptionist_dashboard.router, prefix="/receptionist", tags=["receptionist"])', 'router.include_router(receptionist.router, prefix="/receptionist", tags=["receptionist"])')
else:
    if 'from app.api.v1 import receptionist' not in content:
        content = content.replace('from app.api.v1 import auth', 'from app.api.v1 import auth\nfrom app.api.v1 import receptionist')
