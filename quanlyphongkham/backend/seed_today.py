"""
Script để đảm bảo có dữ liệu cho hôm nay (appointments, waiting patients, billing, audit logs).
Chạy: python seed_today.py
"""
import asyncio
import uuid
import random
...
async def seed_today_data(db: AsyncSession):
    print("🌱 Bắt đầu kiểm tra và tạo dữ liệu cho hôm nay...")
    today = date.today()
    ...
    if not doctors:
        print("Không có doctor nào, vui lòng chạy seed.py trước.")
        return

    result = await db.execute(select(Patient))
    patients = result.scalars().all()
    if not patients:
        print("Không có patient nào, vui lòng chạy seed.py trước.")
        return
    ...
    if len(today_appointments) < 5:
        print(f"Chỉ có {len(today_appointments)} lịch hẹn hôm nay. Đang tạo thêm...")
        ...
        await db.flush()
        print("Đã tạo thêm lịch hẹn.")
    else:
        print("Đã có đủ lịch hẹn cho hôm nay.")
    ...
    if not waiting and today_appointments:
        ...
        apt.checked_in_at = datetime.now(timezone.utc)
        print(f"Đã cập nhật lịch hẹn {apt.appointment_code} thành WAITING.")
        await db.flush()
    ...
    if len(unpaid_bills) < 2:
        print("Đang tạo thêm hóa đơn UNPAID...")
    ...
    if receptionist_id:
        ...
        if len(logs) < 5:
            print("Đang tạo thêm Audit Logs cho receptionist...")
            for i in range(5):
                log = AuditLog(
                    id=str(uuid.uuid4()),
                    ...
                    details="Cập nhật trạng thái lịch hẹn",
    ...
    await db.commit()
    print("✅ Seed dữ liệu hôm nay hoàn tất!")
