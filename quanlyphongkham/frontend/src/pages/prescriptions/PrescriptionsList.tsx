...
export default function PrescriptionsList() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Quản lý Đơn thuốc</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Mã đơn</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Bệnh nhân</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Bác sĩ kê</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Số thuốc</th>
                  <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Ngày kê</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {filtered.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="px-4 py-3 font-mono font-medium text-slate-900 dark:text-white">{p.prescription_code}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-900 dark:text-white">{p.patient?.full_name ?? 'N/A'}</div>
                      <div className="text-xs text-slate-400">{p.patient?.patient_code}</div>
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{p.doctor?.staff?.full_name ?? 'N/A'}</td>
                    <td className="px-4 py-3">
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          {selected && (
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selected.items.map((item, i) => (
                      <div key={item.id} className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3 text-sm">
              ...
              {selected.notes && (
                <div className="border border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-3 text-sm">
